...
@Component({
  selector: 'app-mastering-golden-hour-photography',
  imports: [RouterLink, RouterLinkActive],
  templateUrl: './mastering-golden-hour-photography.component.html',
  styleUrl: './mastering-golden-hour-photography.component.css',
})
export class MasteringGoldenHourPhotographyComponent {
  blogs:IPosts[]=[
    {
      "id": 1,
...
getRelatedBlogs(blog: IPosts) {
  return this.blogs
    .filter(item => item.category === blog.category && item.id !== blog.id)
    .slice(0, 3);
}

getHeadingId(item: string, content: string): string {
  const index = this.getHeadings(content).indexOf(item);
  return `part${index + 1}`;
}
}
